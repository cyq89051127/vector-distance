 ...
 */
public class SimilarityComputer extends Plugin implements ScriptPlugin {
    private static final Logger logger = LogManager.getLogger(SimilarityComputer.class.getName());
    ...
    private class SimilarityEngine implements ScriptEngine {

        // 通过这两个字段判断是否使用该插件
        private final String _SOURCE_VALUE = "image_source";
        private final String _LANG_VALUE = "similarity_image";
        ...
        public <FactoryType> FactoryType compile(String scriptName, String scriptSource, ScriptContext<FactoryType> context, Map<String, String> params) {
            ...
            ScoreScript.Factory factory = (p, lookup) -> new ScoreScript.LeafFactory() {
                String computer = p.getOrDefault("computer","Euclidean").toString();
                ...
                public ScoreScript newInstance(LeafReaderContext context) throws IOException {
                    return new ScoreScript(p, lookup, context) {
                        ...
                        public double execute() {
                            ...
                            if (computer.equalsIgnoreCase("Cosine")){
                                return CosineDistanceComputer.getScore(inputVector, docVector);
                            }else {
                                return EuclideanDistanceComputer.getScore(inputVector, docVector);
                            }
                        }
                    };
                }
